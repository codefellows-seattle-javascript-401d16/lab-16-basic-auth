...
describe('testing auth router', () => {
  ...
  describe('testing POST /api/signup', () => {
    it('should respond with a token', () => {
      ...
      .then(res => {
        console.log('token we got back', res.body);
        expect(res.status).toEqual(200);
        expect(res.text).toExist;
        expect(res.text.length > 1).toBeTruthy();
      });
    });
    it('should return 400 for bad request if body is missing or invalid', () => {
      return superagent.post(`${API_URL}/api/signup`)
      .catch(res => {
  ...
  describe('testing GET /api/login', () => {
    ...
    it('should return 404 if route has not been registered', () => {
      return superagent.get(`${API_URL}/api/notarealroute`)
      .catch(res => {
